...
#include "lib/safe_unordered_map.h"
#include "lib/ScopedMapInserter.h"
#include "lib/ScopedObject.h"
    ...
    if (!workload_defs_file) {
        ...
        if (line.compare("USE_INTERNAL_PARAMS") != 0) {
            ...
            if (mqsim_io_scenarios != NULL) {
                for (auto xml_io_scenario = mqsim_io_scenarios->first_node(
                        "IO_Scenario"); xml_io_scenario; xml_io_scenario = xml_io_scenario->next_sibling(
                        "IO_Scenario")) {
...
void collect_results(SSD_Device *ssd, Host_System *host, const char *output_file_path) {
    ...
    for (unsigned int stream_id = 0; stream_id < IO_flows.size(); stream_id++) {
        cout << "Flow " << IO_flows[stream_id]->ID() << " - total requests generated: "
             << IO_flows[stream_id]->Get_generated_request_count()
             << " total requests serviced:" << IO_flows[stream_id]->Get_serviced_request_count() << endl;
        cout << "                   - device response time: " << IO_flows[stream_id]->Get_device_response_time()
             << " (us)"
...
int main(int argc, char *argv[]) {
    ...
    std::vector<std::vector<IO_Flow_Parameter_Set *> *> *io_scenarios = read_workload_definitions(
            workload_defs_file_path);
    int cntr = 1;
    for (auto io_scen = io_scenarios->begin(); io_scen != io_scenarios->end(); io_scen++, cntr++) {
        system_t *handle = _calloc(1, system_t);
        ...
        handle->engine = new MQSimEngine::Engine();
        handle->datastore = new_data_store();

        SCOPED_OBJECT(handle, register_engine, deregister_engine);
        SCOPED_OBJECT(handle, register_datastore, deregister_datastore);
        ...
        handle->ssd = new SSD_Device(&exec_params->SSD_Device_Configuration,
                                    &exec_params->Host_Configuration.IO_Flow_Definitions);//Create SSD_Device based on the specified parameters
        exec_params->Host_Configuration.Input_file_path = workload_defs_file_path.substr(0,
                                                                                         workload_defs_file_path.find_last_of(
                                                                                                 '.'));//Create Host_System based on the specified parameters
        handle->host = new Host_System(&exec_params->Host_Configuration,
                                      exec_params->SSD_Device_Configuration.Enabled_Preconditioning,
                                      ((SSD_Device *) handle->ssd)->Host_interface);

        SCOPED_OBJECT(handle, register_host, deregister_host);
        ((Host_System *) handle->host)->Attach_ssd_device(((SSD_Device *) handle->ssd));
        ...
        PRINT_MESSAGE("Total simulation time: " << duration / 3600 << ":" << (duration % 3600) / 60 << ":"
                                                << ((duration % 3600) % 60))
        PRINT_MESSAGE("");

        PRINT_MESSAGE("Writing results to output file .......");
        collect_results(((SSD_Device *) handle->ssd), ((Host_System *) handle->host),
                        (workload_defs_file_path.substr(0, workload_defs_file_path.find_last_of(".")) + "_scenario_" +
                         std::to_string(cntr) + ".xml").c_str());
    }
