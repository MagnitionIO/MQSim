...
namespace Utils {
    Logical_Address_Partitioning_Unit::Logical_Address_Partitioning_Unit() {
        initialized = false;
        total_pda_no = 0;
        total_lha_no = 0;
    }
    ...
    Logical_Address_Partitioning_Unit::Allocate_logical_address_for_flows(HostInterface_Types hostinterface_type,
        ...
        auto self = new Logical_Address_Partitioning_Unit;

        self->hostinterface_type = hostinterface_type;
        ...
        self->resource_list = new int ***[channel_count];
        bool resource_sharing = false;
        for (flash_channel_ID_type channel_id = 0; channel_id < channel_count; channel_id++) {
            self->resource_list[channel_id] = new int **[chip_no_per_channel];
            for (flash_chip_ID_type chip_id = 0; chip_id < chip_no_per_channel; chip_id++) {
                self->resource_list[channel_id][chip_id] = new int *[die_no_per_chip];
                for (flash_die_ID_type die_id = 0; die_id < die_no_per_chip; die_id++) {
                    self->resource_list[channel_id][chip_id][die_id] = new int[plane_no_per_die];
                    for (flash_plane_ID_type plane_id = 0; plane_id < plane_no_per_die; plane_id++) {
                        self->resource_list[channel_id][chip_id][die_id][plane_id] = 0;
                    }
                }
            }
        }
                ...
                for (flash_chip_ID_type chip_id = 0; chip_id < stream_chip_ids[stream_id].size(); chip_id++) {
                    for (flash_die_ID_type die_id = 0; die_id < stream_die_ids[stream_id].size(); die_id++) {
                        for (flash_plane_ID_type plane_id = 0;
                            ...
                            if (stream_channel_ids[stream_id][channel_id] >= channel_count) { PRINT_ERROR(
                                        "Invalid channel ID specified for I/O flow " << stream_id);
                            }

                            if (stream_chip_ids[stream_id][chip_id] >= chip_no_per_channel) {
                                PRINT_ERROR(
                                        "Invalid chip ID specified for I/O flow " << stream_id);
                            }

                            if (stream_die_ids[stream_id][die_id] >= die_no_per_chip) {
                                PRINT_ERROR(
                                        "Invalid die ID specified for I/O flow " << stream_id);
                            }

                            if (stream_plane_ids[stream_id][plane_id] >= plane_no_per_die) {
                                PRINT_ERROR(
                                        "Invalid plane ID specified for I/O flow " << stream_id);
                            }
                            auto st_chn_id = stream_channel_ids[stream_id][channel_id];
                            auto st_chp_id = stream_chip_ids[stream_id][chip_id];
                            auto st_die_id = stream_die_ids[stream_id][die_id];
                            auto st_pln_id = stream_plane_ids[stream_id][plane_id];
                            self->resource_list[st_chn_id][st_chp_id][st_die_id][st_pln_id]++;
                        }
                    }
                }
            }
        ...
        for (unsigned int stream_id = 0; stream_id < concurrent_stream_no; stream_id++) {
            ...
            self->stream_channel_ids.push_back(channel_ids);
            for (flash_channel_ID_type channel_id = 0;
                 channel_id < stream_channel_ids[stream_id].size(); channel_id++) {
                self->stream_channel_ids[stream_id].push_back(stream_channel_ids[stream_id][channel_id]);
            }

            std::vector <flash_chip_ID_type> chip_ids;
            self->stream_chip_ids.push_back(chip_ids);
            for (flash_chip_ID_type chip_id = 0; chip_id < stream_chip_ids[stream_id].size(); chip_id++) {
                self->stream_chip_ids[stream_id].push_back(stream_chip_ids[stream_id][chip_id]);
            }

            std::vector <flash_die_ID_type> die_ids;
            self->stream_die_ids.push_back(die_ids);
            for (flash_die_ID_type die_id = 0; die_id < stream_die_ids[stream_id].size(); die_id++) {
                self->stream_die_ids[stream_id].push_back(stream_die_ids[stream_id][die_id]);
            }

            std::vector <flash_plane_ID_type> plane_ids;
            self->stream_plane_ids.push_back(plane_ids);
            for (flash_plane_ID_type plane_id = 0; plane_id < stream_plane_ids[stream_id].size(); plane_id++) {
                self->stream_plane_ids[stream_id].push_back(stream_plane_ids[stream_id][plane_id]);
            }
        }
            ...
            for (flash_channel_ID_type channel_id = 0;
                ...
                for (flash_chip_ID_type chip_id = 0; chip_id < stream_chip_ids[stream_id].size(); chip_id++) {
                    for (flash_die_ID_type die_id = 0; die_id < stream_die_ids[stream_id].size(); die_id++) {
                        for (flash_plane_ID_type plane_id = 0;
                             plane_id < stream_plane_ids[stream_id].size(); plane_id++) {
                            lsa_count += (LHA_type) ((block_no_per_plane * page_no_per_block * sector_no_per_page *
                                                      (1.0 - overprovisioning_ratio) *
                                                      1.0 /
                                                      double(self->resource_list[stream_channel_ids[stream_id][channel_id]][stream_chip_ids[stream_id][chip_id]][stream_die_ids[stream_id][die_id]][stream_plane_ids[stream_id][plane_id]])));
                        }
                    }
                }
            }
            self->pdas_per_flow.push_back(LHA_type(double(lsa_count) / (1.0 - overprovisioning_ratio)));
            self->total_pda_no += self->pdas_per_flow[stream_id];
            lsa_count_per_stream.push_back(lsa_count);
        }

        self->total_lha_no = 0;
        for (unsigned int stream_id = 0; stream_id < concurrent_stream_no; stream_id++) {
            self->start_lhas_per_flow.push_back(self->total_lha_no);
            self->end_lhas_per_flow.push_back(self->total_lha_no + lsa_count_per_stream[stream_id] - 1);
            self->total_lha_no += lsa_count_per_stream[stream_id];
        }
    ...
    LHA_type Logical_Address_Partitioning_Unit::Get_total_device_lha_count() const {
        return total_lha_no;
    }
}
