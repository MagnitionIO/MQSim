...
namespace SSD_Components {
    ...
    public:
        ...
        [[nodiscard]] std::string str() const {
            ...
            st << "**DieBookKeepingEntry** Free:" << Free << " Suspended: " << Suspended
               << " Expected_finish_time: " << Expected_finish_time << " RemainingExecTime: " << RemainingExecTime
               << " DieInterleavedTime: " << DieInterleavedTime << "\n";
            return st.str();
        }
        ...
        void PrepareSuspend() {
            ...
            SuspendedTransactions.insert(SuspendedTransactions.begin(), ActiveTransactions.begin(),
        ...
        void PrepareResume() {
            ...
            ActiveTransactions.insert(ActiveTransactions.begin(), SuspendedTransactions.begin(),
                                      SuspendedTransactions.end());
            Suspended = false;
            SuspendedCommand = nullptr;
    ...
    public:
        ...
        [[nodiscard]] std::string str() const {
            std::ostringstream st;
            st << "**ChipBookKeepingEntry** Status: " << (int) Status
               << " Expected_command_exec_finish_time: " << Expected_command_exec_finish_time
               << " Last_transfer_finish_time: " << Last_transfer_finish_time
               << " HasSuspend: " << HasSuspend << " WaitingReadTXCount: " << WaitingReadTXCount
               << " No_of_active_dies: " << No_of_active_dies << "\n";
            return st.str();
        }
    ...
    public:
        NVM_PHY_ONFI_NVDDR2(const sim_object_id_type &id, ONFI_Channel_NVDDR2 **channels,
                            unsigned int ChannelCount, unsigned int chip_no_per_channel, unsigned int DieNoPerChip,
        ...
        void Send_command_to_chip(std::list<NVM_Transaction_Flash *> &transactionList) override;

        void Change_flash_page_status_for_preconditioning(const NVM::FlashMemory::Physical_Page_Address &page_address,
                                                          const LPA_type lpa) override;

        void Execute_simulator_event(MQSimEngine::Sim_Event *) override;

        BusChannelStatus Get_channel_status(flash_channel_ID_type channelID) override;

        NVM::FlashMemory::Flash_Chip *Get_chip(flash_channel_ID_type channel_id, flash_chip_ID_type chip_id) override;

        LPA_type Get_metadata(flash_channel_ID_type channe_id, flash_chip_ID_type chip_id, flash_die_ID_type die_id,
                              flash_plane_ID_type plane_id, flash_block_ID_type block_id,
                              flash_page_ID_type page_id) override;//A simplification to decrease the complexity of GC execution! The GC unit may need to know the metadata of a page to decide if a page is valid or invalid.
        bool HasSuspendedCommand(NVM::FlashMemory::Flash_Chip *chip) override;

        ChipStatus GetChipStatus(NVM::FlashMemory::Flash_Chip *chip) override;

        sim_time_type Expected_finish_time(NVM::FlashMemory::Flash_Chip *chip) override;
        ...
        void Change_memory_status_preconditioning(const NVM::NVM_Memory_Address *address, const void *status_info) override;

    private:
        void transfer_read_data_from_chip(ChipBookKeepingEntry *chipBKE, DieBookKeepingEntry *dieBKE,
        ...
        static void
        handle_ready_signal_from_chip(MQSimEngine::Sim_Object *instance, NVM::FlashMemory::Flash_Chip *chip, NVM::FlashMemory::Flash_Command *command);
